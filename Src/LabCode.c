...
void allPassFilter(uint32_t *samples, uint32_t *output, int samplesLength,
                   int sampleRate, float32_t decayFactor);
...
void ProcessBuffer() {
  ...
  if (activateDelay) {
    ...
    combFilter(audio_chL, audio_chL_Del, samples_length, delay_delay_time,
               delay_loop_gain, Fs);
    combFilter(audio_chR, audio_chR, samples_length, delay_delay_time,
               delay_loop_gain, Fs);
  ...
  if (activateReverb) {
    uint32_t inLeft_Comb0[BUFFER_SIZE / 2], inLeft_Comb1[BUFFER_SIZE / 2],
        inLeft_Comb2[BUFFER_SIZE / 2], inLeft_Comb3[BUFFER_SIZE / 2];
    uint32_t inRight_Comb0[BUFFER_SIZE / 2], inRight_Comb1[BUFFER_SIZE / 2],
        inRight_Comb2[BUFFER_SIZE / 2], inRight_Comb3[BUFFER_SIZE / 2];
    uint32_t inLeft_Comb[BUFFER_SIZE / 2];
    uint32_t inRight_Comb[BUFFER_SIZE / 2];

    // parallel comb filters left channel
    combFilter(audio_chL, inLeft_Comb0, samples_length, comb0_delay_time,
               comb0_loop_gain, Fs);
    combFilter(audio_chL, inLeft_Comb1, samples_length, comb1_delay_time,
               comb1_loop_gain, Fs);
    combFilter(audio_chL, inLeft_Comb2, samples_length, comb2_delay_time,
               comb2_loop_gain, Fs);
    combFilter(audio_chL, inLeft_Comb3, samples_length, comb3_delay_time,
               comb3_loop_gain, Fs);

    // parallel comb filters right channel
    combFilter(audio_chR, inRight_Comb0, samples_length, comb0_delay_time,
               comb0_loop_gain, Fs);
    combFilter(audio_chR, inRight_Comb1, samples_length, comb1_delay_time,
               comb1_loop_gain, Fs);
    combFilter(audio_chR, inRight_Comb2, samples_length, comb2_delay_time,
               comb2_loop_gain, Fs);
    combFilter(audio_chR, inRight_Comb3, samples_length, comb3_delay_time,
               comb3_loop_gain, Fs);

    // summing up combfilter signals
    for (int i = 0; i < seperated_buf_index; i++) {
      inLeft_Comb[i] =
          inLeft_Comb0[i] + inLeft_Comb1[i] + inLeft_Comb2[i] + inLeft_Comb3[i];
      inRight_Comb[i] = inRight_Comb0[i] + inRight_Comb1[i] + inRight_Comb2[i] +
                        inRight_Comb3[i];
    }

    // Serial allpass filters left
    allPassFilter(inLeft_Comb, inLeft_Comb, samples_length, Fs,
                  allpass0_loop_gain);
    allPassFilter(inLeft_Comb, inLeft_Comb, samples_length, Fs,
                  allpass0_loop_gain);

    // Serial allpass filters right
    allPassFilter(inRight_Comb, inRight_Comb, samples_length, Fs,
                  allpass1_loop_gain);
    allPassFilter(inRight_Comb, inRight_Comb, samples_length, Fs,
                  allpass1_loop_gain);
  }
...
void Init() { // 48kHz -> FS auch aendern
  ...
  for (int i = 0; i < samplesLength; i++) {
    ...
    if (i - delaySamples >= 0) {
      allPassFilterSamples[i] +=
          -1 * decayFactor * allPassFilterSamples[i - delaySamples];
    }
    if (i - delaySamples >= 1) {
      allPassFilterSamples[i] +=
          decayFactor * allPassFilterSamples[i + 20 - delaySamples];
    }
  }
