...
void Display_FFT()
{
  ...
  // Plot the magnitude of the FFT, plot only the FFT values from f=0 to f=r/2, where r is the sampling rate
  Plot_Signal_Float(display_buffer, FFT_SIZE/2, 0.0, SamplingRateHz/2.0, 0.0, ymax, fft_title, 1000.0f);
}
...
void Display_FFT2()
{
  ...
  ymax =  display_buffer[0] > display_buffer[1] ? display_buffer[0] : display_buffer[1];
  ...
  // Plot the magnitude of the FFT, plot only the FFT values from f=0 to f=r/2, where r is the sampling rate
  Plot_Signal2_Float(display_buffer, FFT_SIZE, 0.0, SamplingRateHz/2.0, 0.0, ymax, fft_title, 1000.0f);
}
...
void Display_Signal(uint16_t channel)
{
  ...
  Plot_Signal_Float(display_buffer, FFT_SIZE, 0.0, xmax, ymin, ymax,"Time signal", xmax*0.1f);
}
...
void Display_Signal2()
{
  ...
  Plot_Signal2_Float(fft_buffer, BUFFER_SIZE, 0.0, xmax, ymin, ymax, "Time signal", xmax*0.1f);
}


void Plot_Signal_Fast(float32_t *samples, uint16_t num_samples, float32_t xmin, float32_t xmax, float32_t ymin, float32_t ymax, const char *title, float32_t xdiv)
{
  char s[30];
  ...
  BSP_LCD_DisplayStringAt(display_left_margin, display_max_y+display_top_margin+display_bottom_margin-14 ,(uint8_t*)s, LEFT_MODE);
  // display xmax
  snprintf(s,15,"xmax=%-6f", xmax);
  BSP_LCD_SetFont(&Font16);
  BSP_LCD_DisplayStringAt(0, display_max_y+display_top_margin+display_bottom_margin-14 ,(uint8_t*)s, RIGHT_MODE);
  // display ymin
  snprintf(s,15,"ymin=%-6f", ymin);
  BSP_LCD_SetFont(&Font16);
  BSP_LCD_DisplayStringAt(display_left_margin, display_max_y+display_top_margin ,(uint8_t*)s, LEFT_MODE);
  // display ymax
  ...
  // display_top_margin + display_max_y - (uint16_t)((y-ymin)/(ymax-ymin)*display_max_y)
  // compute y-ymin vectorized in-place using ARM CMSIS function
  ...
  // multiply with -display_max_y/(ymax-ymin) in-place vectorized using ARM CMSIS function
  ...
  // add display_top_margin + display_max_y vectorized in-place using ARM CMSIS function
  // arm_offset_f32 (float32_t *pSrc, float32_t offset, float32_t *pDst, uint32_t blockSize)
  arm_offset_f32(samples, (float32_t)(display_top_margin + display_max_y), samples, num_samples);
  // draw x-axis =========================================================================
  // use blue color
  ...
  {
    // BSP_LCD_DrawLine(X1, Y1, X2, Y2): Draw line between to points (X1,Y1) and (X2, Y2),
    //                                   where (0,0) is upper, left of display
    BSP_LCD_DrawLine(display_x(xmin, xtickval, xmax), display_y(ymin, y_xaxis, ymax)-2,
  ...
  if (num_samples < display_max_x)
  {
    for(i=1;i<num_samples;i++)
    {
      // BSP_LCD_DrawLine(X1, Y1, X2, Y2): Draw line between to points (X1,Y1) and (X2, Y2),
      //                                   where (0,0) is upper, left of display
      BSP_LCD_DrawLine(display_x(0, i-1, num_samples), (uint16_t)samples[i-1],
                       display_x(0, i, num_samples),   (uint16_t)samples[i]);
    }
  }
  else
  {
...
void Plot_Signal_Float(float32_t *samples, uint16_t num_samples, float32_t xmin, float32_t xmax, float32_t ymin, float32_t ymax, const char *title, float32_t xdiv)
{
  ...
  if (num_samples < display_max_x)
  {
    previous_sample = display_top_margin + display_max_y
...
void Plot_Signal2_Fast(float32_t *samples, uint16_t num_samples, float32_t xmin, float32_t xmax, float32_t ymin, float32_t ymax, const char *title, float32_t xdiv)
{
  ...
  BSP_LCD_DisplayStringAt(display_left_margin, 10,(uint8_t*)s, LEFT_MODE);
  // convert y-values to display y values ======================================
  // display_y = top_margin + display_max_y - (uint16_t)((y-ymin)/(ymax-ymin)*display_max_y)
  // compute y-ymin vectorized in-place using ARM CMSIS function
  // arm_offset_f32 (float32_t *pSrc, float32_t offset, float32_t *pDst, uint32_t blockSize)
  arm_offset_f32(samples, -ymin, samples, num_samples);
  // multiply with -display_max_y/(ymax-ymin) in-place vectorized using ARM CMSIS function
  ...
  if (num_samples/2 < display_max_x)
  {
    ...
    for(i=2;i<num_samples;i+=2)
    {
      // BSP_LCD_DrawLine(X1, Y1, X2, Y2): Draw line between to points (X1,Y1) and (X2, Y2),
      //                                   where (0,0) is upper, left of display
      BSP_LCD_DrawLine(display_x(0, i-2, num_samples), (uint16_t)samples[i-2],
                       display_x(0, i, num_samples),   (uint16_t)samples[i]);
    }
    ...
    for(i=3;i<num_samples;i+=2)
    {
        // BSP_LCD_DrawLine(X1, Y1, X2, Y2): Draw line between to points (X1,Y1) and (X2, Y2),
        //                                   where (0,0) is upper, left of display
        BSP_LCD_DrawLine(display_x(0, i-3, num_samples), (uint16_t)samples[i-2],
...
void Plot_Signal2_Float(float32_t *samples, uint16_t num_samples, float32_t xmin, float32_t xmax, float32_t ymin, float32_t ymax, const char *title, float32_t xdiv)
{
  ...
  if (num_samples/2 < display_max_x)
  {
    // set the (text) color for the first plot
    BSP_LCD_SetTextColor(LCD_COLOR_RED);
    previous_sample_1 = display_top_margin + display_max_y
        - (uint16_t)((samples[0]-ymin)/(ymax-ymin)*display_max_y);
    for(i=2;i<num_samples;i+=2)
    {
      sample = display_top_margin + display_max_y
...
{
  return (uint16_t)((x-xmin)/(xmax-xmin)*display_max_x) + display_left_margin;
}

uint16_t display_y(float32_t ymin, float32_t y, float32_t ymax)
{
  return display_top_margin + display_max_y - (uint16_t)((y-ymin)/(ymax-ymin)*display_max_y);
}
