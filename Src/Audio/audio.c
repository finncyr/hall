 ...
 * 						When only line is used the samples come from
 * the line in left channel. When using mic in only the samples come from the
 * left microphone When both inputs are used the samples come from the left
 * microphone
 * @param	inRight		sample from the right channel.
 * 						When only line is used the samples come from
 * the line in right channel. When using mic in only the samples come from the
 * right microphone When both inputs are used the samples come from the right
 * microphone
 * @param 	outLeft		Pointer to sample storage for the left channel
 * 						All the time this is the output value for the
 * left channel When both inputs are used also the in samples are provided in
 * this storage
 * @param 	outRight	Pointer to sample storage for the right channel
 * 						All the time this is the output value for the
 * right channel When both inputs are used also the in samples are provided in
 * this storage
 */
void (*ptrSample_Callback)(uint32_t inLeft, uint32_t inRight, uint32_t *outLeft,
                           uint32_t *outRight) = NULL;
...
void SAI_IT(SAI_HandleTypeDef *hsai) {
  if (((hsai_BlockB1.Instance->SR & SAI_xSR_FLVL) >> 16) > 1) {
    ...
    if (input & WM8994_INPUT_SLOT2) // When the microphones are switched on time
                                    // slot 1 is also used
    {
...
/**
 * @brief   Set the mupliplicator and divisor settings structure for the PLLI2S
 * and SAI2 peripherals according to the frequency selection. The output of
 * divisor PLLM must be 1MHz.
 *
 * @param   frequencySelection  Selected frequency, this should be specified
 * through a WM8994_FREQ_... define
 * @param   config              Struct in which the parameter values will be
 * written
 */
void SetClockConfigurationParameter(uint8_t frequencySelection,
...
/**
 * @brief   Changes multiplicator and divisor settings of the PLLI2S and SAI2
 * peripherals
 *
 ...
 *
 * @retval  Returns HAL_OK if the changes could be aplied in case of a time out
 * HAL_TIMEOUT will be returned
 * @remark  In case of a failure in configuring the SAI the system error handle
 * will be called
 */
HAL_StatusTypeDef ReconfigureSAIClock(SAI_CLOCK_CONFIG_t *config,
 ...
 *
 * @param selectedInput     Selected input, specified through WM8994_INPUT_...
 * defines or line_in and/or mic_in defines
 * @param selectedFrequency Selected sample rate, specified through
 * WM8994_FREQ_...K defines or hz..... defines
 ...
 */
void AudioInitDMA(const uint8_t selectedFrequency, const uint8_t selectedInput,
                  void (*half_Callback)(), void (*full_Callback)()) {
  ...
  HAL_SAI_Transmit_DMA(&hsai_BlockA1, (uint8_t *)bufferTransmit,
                       BUFFER_SIZE * 2);
 ...
 */
HAL_StatusTypeDef
AudioInitIT(const uint8_t selectedFrequency, const uint8_t selectedInput,
            void (*sample_Callback)(uint32_t inLeft, uint32_t inRight,
  ...
  __HAL_SAI_ENABLE_IT(&hsai_BlockA1,
                      SAI_IT_OVRUDR | SAI_IT_FREQ |
                          SAI_IT_WCKCFG); // TODO maybe remove the other two
